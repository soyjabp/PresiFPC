...
function mostrarModalSimulacion() {
  ...
  const contenidoTexto = document.getElementById("modalContenidoTexto");
  const contenidoTabla = document.getElementById("modalContenidoTabla");
  ...
  if (indiceMensaje < mensajesSimulacion.length) {
    ...
    contenidoTexto.innerHTML = `<pre>${msg.texto}</pre>`;
    ...
    contenidoTabla.innerHTML = mostrarTabla(
        "",
        msg.tabla,
        "",
        false,
...
function cerrarModalSimulacion() {
    ...
    // Mostrar la fase final una vez terminó el recorrido del modal

    const usarPlayoffs =
        document.getElementById("modoPlayoffs")?.checked;

    mostrarTabla(
    "tct",
    tabla,
    `Temporada ${temporadaActual} - Semestre ${semestreActual}`,
    true,
    [],
    false
);
    ...
    document.getElementById("btnSiguientes").textContent = "Siguiente";
...
let resultadosCuadrangulares = null;
let partidosCuadrangulares = null;
...
function simularCuadrangulares() {
  ...
  ["A", "B"].forEach(g => {
    ...
    calendario.forEach((fechaPartidos, fechaIndex) => {
      ...
      //alert(texto.trim());
      const tablaGrupo = JSON.parse(JSON.stringify(resultados[g]));
    
    mensajesSimulacion.push({
    titulo: `🏆 Cuadrangulares - Grupo ${g} - Fecha ${fechaIndex + 1}`,
    texto: texto.trim()
});
...
mensajesSimulacion.push({
    titulo: `📊 Tabla Grupo ${g} - Fecha ${fechaIndex + 1}`,
    texto: "",
    tabla: tablaGrupo
});
    });
...
resultadosCuadrangulares = resultados;
partidosCuadrangulares = partidosGrupos;
}

function mostrarCuadrangulares() {

    if (!resultadosCuadrangulares) return;
    ...
    mostrarTabla(
        "grupoA",
        resultadosCuadrangulares.A,
        "Grupo A",
        false,
        [resultadosCuadrangulares.A[0].nombre],
        false
    );

    mostrarTabla(
        "grupoB",
        resultadosCuadrangulares.B,
        "Grupo B",
        false,
        [resultadosCuadrangulares.B[0].nombre],
        false
    );

    mostrarPartidos(
        "grupoA",
        partidosCuadrangulares.A
    );

    mostrarPartidos(
        "grupoB",
        partidosCuadrangulares.B
    );
...
function simularPlayoffsNuevo() {
  ...
  mensajesSimulacion.push({
    ...
    texto:`
${rutaA[0]} ${marcador(rutaA[0],rutaA[1],cuartosA1,rA1)} ${rutaA[1]}<br>
${rutaA[2]} ${marcador(rutaA[2],rutaA[3],cuartosA2,rA2)} ${rutaA[3]}<br><br>

${rutaB[0]} ${marcador(rutaB[0],rutaB[1],cuartosB1,rB1)} ${rutaB[1]}<br>
${rutaB[2]} ${marcador(rutaB[2],rutaB[3],cuartosB2,rB2)} ${rutaB[3]}
`
});
...
mensajesSimulacion.push({
    titulo:"🏆 Semifinales",
    texto:`
${rA1.ganador} ${marcador(rA1.ganador,rA2.ganador,semiA,sA)} ${rA2.ganador}<br>

${rB1.ganador} ${marcador(rB1.ganador,rB2.ganador,semiB,sB)} ${rB2.ganador}
`
});
...
audio.play();
  ...
  mensajesSimulacion.push({
    ...
    texto: `
        ${escudo1}
        ${equipo1.nombre} ${ida.golesA} - ${ida.golesB}
        ${equipo2.nombre}
        ${escudo2}
    `
});
...
mensajesSimulacion.push({
    ...
    texto: `
        ${escudo2}
        ${equipo2.nombre} ${vuelta.golesA} - ${vuelta.golesB}
        ${equipo1.nombre}
        ${escudo1}
    `
});
...
function mostrarFinal() {
    ...
    document.getElementById("final").innerHTML = `
        ...
        <h3>🏆 ${escudoCampeon} Campeón: ${campeon}</h3>
    `;
