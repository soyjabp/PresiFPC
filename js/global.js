...
//PRIMERA 
let equiposPrimera = [
  { nombre: "Nacional", fuerza: 73},
  { nombre: "Junior", fuerza: 71 },
  { nombre: "Millonarios", fuerza: 71 },
  { nombre: "América", fuerza: 70 },
  { nombre: "Santa Fe", fuerza: 70 },
  { nombre: "Medellín", fuerza: 70 },
  { nombre: "Tolima", fuerza: 69 },
  { nombre: "Cali", fuerza: 68 },
  { nombre: "Once Caldas", fuerza: 68},
  { nombre: "Bucaramanga", fuerza: 67 },
  { nombre: "Pasto", fuerza: 66 },
  { nombre: "Inter Bogotá", fuerza: 65},
  { nombre: "Águilas", fuerza: 65 },
  { nombre: "Fortaleza", fuerza: 65 },
  { nombre: "Pereira", fuerza: 65 },
  { nombre: "Alianza", fuerza: 64 },
...
function seleccionarEquipo(nombreEquipo) {
 ...
 //fuerzas
  ...
  document.getElementById("fuerzaEquipo").textContent = fuerza;

  let base = obtenerFuerzaEquipo(equipoUsuario);
  document.getElementById("baseEquipo").innerText = `Base: ${base}`;
  ...
  document.getElementById("equipo").textContent = equipoUsuario;
  document.getElementById("escudo").src = getEscudoEquipo(equipoUsuario);

  document.getElementById("selectEquipoRefundar").disabled = true;
  document.getElementById("selectEquipoSegunda").disabled = true;
}
...
function obtenerFuerzaTotal(nombreEquipo) {
  ...
  //Obtener los 11 titulares reales según esquema y posiciones
  const alineacion = obtenerTitulares(nombreEquipo);

  if (!alineacion || !alineacion.titulares || alineacion.titulares.length < 11) {
    return base;
  }

  const titulares = alineacion.titulares;

  const mediaXI = titulares.reduce((sum, j) => {
  const media = Number(j.media) || 0;
  const forma = Number(j.forma) || 0;
  return sum + (media + forma * 0.4);
}, 0) / titulares.length;

document.getElementById("titular").innerText = `Titular: ${mediaXI.toFixed(2)}`;

  const mediatotal = plantilla.reduce((sum, j) => {
  const media = Number(j.media) || 0;
  const forma = Number(j.forma) || 0;
  return sum + (media + forma * 0.4);
}, 0) / plantilla.length;

 document.getElementById("mediaEquipo").innerText = `Plantilla: ${mediatotal.toFixed(2)}`;

const pesoBase = 0.30;
const pesoMedia = 0.70;

  let fuerzaTotal = Math.round(base * pesoBase + mediaXI * pesoMedia);
...
function Simular(){
  ...
  simularTorneoB(semestreActual);

  setTimeout(simularTCT,3000);
}
...
function simularPartido(equipoA, equipoB) {
  ...
  const ventaja = Math.tanh(diferencia / 12);
  ...
  //const plantillaA = plantillasJugadores[equipoA].slice(0,11);
  //const plantillaB = plantillasJugadores[equipoB].slice(0,11);

  const alineacionA = obtenerTitulares(equipoA);
  const alineacionB = obtenerTitulares(equipoB);

  const plantillaA = alineacionA.titulares;
  const plantillaB = alineacionB.titulares;

  console.log(
    equipoA,
    plantillaA.map(j => `${j.nombre} (${j.posicion})`)
);

console.log(
    equipoB,
    plantillaB.map(j => `${j.nombre} (${j.posicion})`)
);
  ...
  if (equipoUsuario === equipoA || equipoUsuario === equipoB) {
   ...
   if (golesPropios > golesRivales) {
    estadisticasUsuario.victorias++;
    ajustarMoralHinchada("ganado");
    } 
   else if (golesPropios < golesRivales) {
    estadisticasUsuario.derrotas++;
    ajustarMoralHinchada("perdido");
    } 
   else {
    estadisticasUsuario.empates++;
    ajustarMoralHinchada("empatado");
    }
    actualizarMoralHinchadaUI();
    actualizarVEDUI();
  }
...
function elegirGoleadorPorPeso(plantilla) {
  ...
  for (const jugador of plantilla) {
    ...
    const posicion = jugador.posicion?.toUpperCase() || "MO";
    ...
    // Bonus por calidad
    const bonusMedia = Math.floor((media - 65) / 3);

    let pesoFinal =
    pesoBase +
    bonusMedia +
    Math.round(forma*0.5);
...
function actualizarVEDUI() {
    const elemento = document.getElementById("vedUsuario");

    if (!elemento) return;

    const { victorias, empates, derrotas } = estadisticasUsuario;
...
function mostrarTabla(divId, datos, titulo, destacarTop8 = false, lideres = [], marcarDescenso = false,  devolverHTML = false) {
  const modoMX = document.getElementById("modoMX")?.checked;

  let html = `<h3>${titulo}</h3><table><tr><th>#</th><th>Equipo</th><th>PTS</th><th>DG</th></tr>`;
...
function renderEquipos() {
  ...
  // Siempre obtiene la lista actualizada
  const todosLosEquipos = obtenerTodosLosEquipos();

  let nombreEquipo = "Selecciona un equipo";

  if (indiceEquipo >= 0 && todosLosEquipos[indiceEquipo]) {
    nombreEquipo = todosLosEquipos[indiceEquipo].nombre;
  }
  ...
  // Solo mostrar plantilla si hay equipo seleccionado
  if (indiceEquipo >= 0 && todosLosEquipos[indiceEquipo]) {
    mostrarPlantilla(
      todosLosEquipos[indiceEquipo].nombre
    );
  }
}

function cambiarEquipo(direccion) {
  // Obtener siempre los equipos actuales
  const todosLosEquipos = obtenerTodosLosEquipos();

  // Seguridad por si no hay equipos
  if (todosLosEquipos.length === 0) return;


  // Al inicio no hay equipo seleccionado
  if (indiceEquipo === -1) {

    if (direccion === 1) {
      indiceEquipo = 0;
    } else {
      indiceEquipo = todosLosEquipos.length - 1;
    }

  } else {

    indiceEquipo += direccion;

    // Si retrocede desde el primero
    if (indiceEquipo < 0) {
      indiceEquipo = todosLosEquipos.length - 1;
    }

    // Si avanza desde el último
    if (indiceEquipo >= todosLosEquipos.length) {
      indiceEquipo = 0;
    }
  }
...
function refundarClub() {
  ...
  equiposB = [...equiposSegunda];
  indiceEquipoB = -1;

  // 3️⃣ Plantilla vacía
  //plantillasJugadores[equipoNuevo] = [];
  plantillasJugadores[equipoNuevo] = generarPlantillaEquipo(equipoNuevo);
  ...
  agregarNotificacion(`🏟️ ${equipoNuevo} ha sido refundado y entra a la B en la Temporada ${temporadaActual}-${semestreActual}`);
}
...
const inputNombre = document.getElementById("nombrePresidente");
const inputFoto = document.getElementById("foto");

const btnGuardar = document.getElementById("guardarPerfil");

const preview = document.getElementById("preview");
const nombreMostrar = document.getElementById("nombrePresidenteMostrar");
...
btnGuardar.addEventListener("click", () => {
    ...
    lector.onload = function(e){
        ...
        localStorage.setItem("fotoPresidente", imagenBase64);

        nombreMostrar.textContent = `Bienvenido: ${nombre}`;
...
function actualizarTemporada() {
    document.getElementById("temporada").textContent = temporadaActual;
    document.getElementById("semestre").textContent = semestreActual;
}
...
const FONDO_SIN_LICENCIA = "img/fondos/fondo.png";
const FONDO_CON_LICENCIA = "img/fondos/premium.png";
...
function getEscudoEquipo(nombre) {
  ...
  if (esColombiano) {
    ...
    return `img/escudos/${limpio}.png`;
  }

  // 🌎 LIBERTADORES (SIEMPRE visibles)
  return `img/escudos-lib/${limpio}.png`;
}
...
window.onload = () => {
  ...
  const nombre = localStorage.getItem("nombrePresidente");
  const foto = localStorage.getItem("fotoPresidente");
